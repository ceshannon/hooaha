package com.hooaha.andr.im.zuzhii.fragment;
...
import com.hooaha.andr.im.zuzhii.ShowMessageActivity;
...
import com.hooaha.andr.im.zuzhii.view.DialogView;
    ...
    public void getData() {
        ...
        new Thread() {
            ...
            public void run() {
                ...
                groups = ZuzhiiXMPP.getInstance().getGroups();
                childs = new HashMap<RosterGroup, List<XmppFriend>>();
                HashMap<String, Object> map = XmppService.map;
                for (int i = 0; i < groups.size(); i++) {
                    List<RosterEntry> child = ZuzhiiXMPP.getInstance().getEntrysByGroup(groups.get(i).getName());
                    List<XmppFriend> lists = new ArrayList<XmppFriend>();
                    list_is_open.add(groups.get(i).getName());
                    for (int j = 0; j < child.size(); j++) {
                        List<XmppUser> list = ZuzhiiXMPP.getInstance().searchUsers(child.get(j).getUser().split("@")[0]);
